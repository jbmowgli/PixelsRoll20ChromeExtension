...
describe('Connection Monitoring and Cleanup', () => {
  ...
  describe('Connection State Tracking', () => {
    ...
    test('should handle multiple pixel connections', async () => {
      ...
      // Connect first pixel
      mockBluetooth.requestDevice.mockResolvedValueOnce(createMockDevice('Pixel1'));
      await messageListener({ action: 'connect' }, null, jest.fn());
      
      // Connect second pixel
      mockBluetooth.requestDevice.mockResolvedValueOnce(createMockDevice('Pixel2'));
      await messageListener({ action: 'connect' }, null, jest.fn());
  ...
  describe('Cleanup Operations', () => {
    test('should implement global cleanup logic', () => {
      ...
      const cleanupFunction = intervalSpy.mock.calls
        .find(call => call[1] === 60000)?.[0];
  ...
  describe('Per-Pixel Connection Monitoring', () => {
    ...
    test('should detect connection loss', async () => {
      ...
      const monitoringFunction = intervalSpy.mock.calls
        .find(call => call[1] === 30000)?.[0];
